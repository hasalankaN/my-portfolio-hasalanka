...
const ProfileImageUpload: React.FC<Props> = ({ imgURL }) => {
  ...
  const simulateUpload = async () => {
    ...
    if (originalImage && croppedAreaPixels) {
      ...
      if (res.status === "FAIL") {
        toast(res.message || "Failed to update profile picture");
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="relative cursor-pointer" onClick={handleAvatarClick}>
        ...
        <i className="profile-image-edit-icon size-[31px] absolute bottom-0 right-0 border-white border-[1.5px] rounded-full" />
      </div>
      <input
        type="file"
        ref={inputRef}
